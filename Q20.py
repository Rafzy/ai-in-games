...
class Quoridor:
    def __init__(self, board_size):
        ...
        self.player_positions = {
            ...
            "P2": (0, board_size // 2 if board_size % 2 == 1 else board_size // 2 - 1),
        }
        ...
        self.previous_player_positions = {
            "P1": None,
            "P2": None,
        }  # Initialize the previous_player_positions attribute
    ...
    def get_legal_moves(self):
        ...
        if self.walls[self.players[self.ply]] > 0:
            for i in range(0, self.board_size - 1):
                for j in range(0, self.board_size - 1):
                    ...
                    if (
                        self.board[i][j] != "H"
                        and self.board[i][j] != "HH"
                        and self.board[i][j] != "VV"
                        and self.board[i][j] != "HV"
                        and self.board[i][j + 1] != "H"
                        and self.board[i][j + 1] != "HH"
                        and self.board[i][j + 1] != "HV"
                    ):
                        move = ("H", i, j)
                        ...
                        reachable2 = self.reachable(
                            self.player_positions["P2"], (board_size - 1, 0)
                        )
                    ...
                    if (
                        ...
                        and self.board[i][j] != "HH"
                        and self.board[i][j] != "HV"
                        and self.board[i + 1][j] != "V"
                        and self.board[i + 1][j] != "VV"
                        and self.board[i + 1][j] != "HV"
                    ):
                        move = ("V", i, j)
                        # simulate placing a wall to compute path existance
                        previous = self.update_board_wall(move)
        ...
        if (
            ...
            and self.board[current_position[0] - 1][current_position[1]] != "H"
            and self.board[current_position[0] - 1][current_position[1]] != "HH"
            and self.board[current_position[0] - 1][current_position[1]] != "HV"
        ):
            moves.append(("U",))
        # Check if the player can move down
        if (
            current_position[0] < self.board_size - 1
            and self.board[current_position[0]][current_position[1]] != "H"
            and self.board[current_position[0]][current_position[1]] != "HH"
            and self.board[current_position[0]][current_position[1]] != "HV"
        ):
            moves.append(("D",))
        # Check if the player can move left
        if (
            current_position[1] > 0
            and self.board[current_position[0]][current_position[1] - 1] != "V"
            and self.board[current_position[0]][current_position[1] - 1] != "VV"
            and self.board[current_position[0]][current_position[1] - 1] != "HV"
        ):
            moves.append(("L",))
        # Check if the player can move right
        if (
            current_position[1] < self.board_size - 1
            and self.board[current_position[0]][current_position[1]] != "V"
            and self.board[current_position[0]][current_position[1]] != "VV"
            and self.board[current_position[0]][current_position[1]] != "HV"
        ):
            moves.append(("R",))
        return moves
    ...
    def extract_path(self, start_position, target_position):
        ...
        while current[0] != target_position[0]:
            ...
            for direction in directions:
                ...
                if neighbor == start_position or (
                    neighbor[0] >= 0
                    and neighbor[0] < self.board_size
                    and neighbor[1] >= 0
                    and neighbor[1] < self.board_size
                ):
                    # print("in bounds")
                    if direction == (0, -1):
                        forward_move = ("L",)
    ...
    def move_player(self, move):
        ...
        self.previous_player_positions[current_player] = (
            current_position  # Store the previous player position
        )
        if move[0] == "U":
            self.player_positions[current_player] = (
        ...
        if (
            current_player == "P1" and self.player_positions[current_player][0] == 0
        ) or (
            current_player == "P2"
            and self.player_positions[current_player][0] == self.board_size - 1
        ):
            if self.game_over == False:
                print(
                    self.player_positions,
                    self.player_positions[current_player][0] == 0,
                    self.player_positions[current_player][0] == self.board_size - 1,
                )
    ...
    def update_board_wall(self, move):
        ...
        previous = (
            ...
            self.board[center_row + 1][center_col],
        )
    ...
    def place_wall(self, move):
        ...
        current_player = self.players[self.ply]
        self.previous_player_positions[current_player] = self.player_positions[
            current_player
    ...
    def place_random_wall(self):
        ...
        for move in legal_moves:
            if (move[0] == "H" and move[1] > 0 and move[1] < board_size - 2) or (
                move[0] == "V" and move[1] > 0 and move[1] < board_size - 2
            ):
    ...
    def undo_move(self):
        ...
        self.previous_player_positions = {
            "P1": None,
            "P2": None,
        }  # Reset the previous player positions
    ...
    def draw_board(self):
        ...
        for i in range(1, self.board_size):
            pygame.draw.line(
                window,
                FRAME_COLOR,
                (i * square_size + FRAME_SIZE, FRAME_SIZE),
                (i * square_size + FRAME_SIZE, HEIGHT + FRAME_SIZE),
                2,
            )
            pygame.draw.line(
                window,
                FRAME_COLOR,
                (FRAME_SIZE, i * square_size + FRAME_SIZE),
                (WIDTH + FRAME_SIZE, i * square_size + FRAME_SIZE),
                2,
            )
            ...
            if position is not None:  # Check if position is not None
                ...
                pygame.draw.circle(
                    ...
                    (
                        ...
                        position[0] * square_size + square_size // 2 + FRAME_SIZE,
                    ),
                    square_size // 4,
                )
            ...
            for j in range(self.board_size):
                if self.board[i][j] == "H" or self.board[i][j] == "HH":
                    pygame.draw.line(
                        ...
                        WALLS_COLOR,
                        (
                            (j) * square_size + FRAME_SIZE,
                            (i + 1) * square_size + FRAME_SIZE,
                        ),
                        (
                            ((j) + 1) * square_size + FRAME_SIZE,
                            (i + 1) * square_size + FRAME_SIZE,
                        ),
                        wall_thickness,
                    )
        ...
        for i in range(self.board_size):
            ...
            window.blit(
                text, (50 + (i + 0.5) * square_size - text.get_width() // 2, 20)
            )

            # Display the letters A-I for rows
            text = font.render(chr(ord("A") + i), True, WALLS_COLOR)
            window.blit(
                text, (20, 50 + (i + 0.5) * square_size - text.get_height() // 2)
            )
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if (
                ...
                or event.key == K_PAGEUP
                or event.key == K_PAGEDOWN
            ):
